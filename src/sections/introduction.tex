%----------------------------------------------------------------------------------------------------------------------%


\section{Introduction}\label{sec:introduction}
%----------------------------------------------------------------------------------------------------------------------%

\subsection{History of running a software}\label{subsec:history-of-running-a-software}
\begin{frame}{History of running a software}
    \begin{itemize}[<+- | alert@+>]
        \item Early on, organizations ran applications on physical servers.
        \item Install or use an existing operating system.
        \item Install the tools used by your software.
        \item Install dependencies of your software.
        \item Run your software.
    \end{itemize}
\end{frame}

\subsection{Traditional Deployment}\label{subsec:traditional-deployment}
\begin{frame}{Traditional Deployment}
    \begin{tikzpicture}[
        node/.style={
            scale=0.75,
            node distance=2mm,
            rectangle,
            draw=black,
            thick,
            text centered,
            minimum height=10mm,
            minimum width=10mm
        }
    ]
        \tikzstyle{node} = [
            scale=0.75,
            node distance=2mm,
            rectangle,
            draw=black,
            thick,
            text centered,
            minimum height=10mm,
            minimum width=10mm
        ]
        \tikzstyle{hw} = [fill=blue!35]
        \tikzstyle{sw} = [fill=blue!15]

        \node [node, sw, text width=1.95cm] (A1) {application};
        \node [node, sw, right = of A1, text width=1.95cm] (A2) {application};
        \node [node, sw, right = of A2, text width=1.95cm] (A3) {application};
        \node [node, hw, below = of A2, text width=7cm] (OS) {Operating System};
        \node [node, hw, below = of OS, text width=7cm] (HW) {Hardware};
    \end{tikzpicture}
\end{frame}


\begin{frame}{Traditional Deployment}
    \begin{tikzpicture}
        \matrix (magic) [matrix of nodes,ampersand replacement=\&, column sep=7mm, row sep=2mm]{
            \node (se) [xshift = -2cm,draw,shape=rectangle,draw=black,
            thick,
            text centered,
            minimum height=10mm,
            minimum width=15mm,fill=blue!35,visible on=<3->] {App 1 };
            \node (yw) [xshift = 1cm,right of=se,draw,shape=rectangle,draw=black,
            thick,
            text centered,
            minimum height=10mm,
            minimum width=15mm,fill=blue!35,visible on=<4->] {App 2};
            \node (ul) [xshift = 1cm,right of=yw,draw,shape=rectangle,draw=black,
            thick,
            text centered,
            minimum height=10mm,
            minimum width=15mm,fill=blue!35,visible on=<5->] {App 3}; \\
            \node (d1) [draw,shape=rectangle,draw=black,
            thick,
            text centered,
            minimum height=10mm,
            minimum width=60mm,fill=blue!35,visible on=<2->] {Operating System}; \\
            \node (pu) [draw,shape=rectangle,draw=black,
            thick,
            text centered,
            minimum height=10mm,
            minimum width=60mm,fill=blue!35,visible on=<1->] {Hardware}; \\
        };
    \end{tikzpicture}
\end{frame}


\subsection{Issues with traditional deployment}\label{subsec:issues-with-traditional-deployment}
\begin{frame}{Issues with traditional deployment}
    \begin{itemize}[<+- | alert@+>]
        \item No way to define resource boundaries for applications in a physical server, and this caused resource allocation issues.
        \item Scaling issues as resources were underutilized.
        \item It was expensive for organizations to maintain many physical servers.
    \end{itemize}
\end{frame}
%----------------------------------------------------------------------------------------------------------------------%